...
def test_put_all_settings_invalid(client, api_prefix):
    all_settings_dict = load_test_data_file("api", "test_put_all_settings_invalid.json")
    response = client.put(f"{api_prefix}/put-all-settings", json=all_settings_dict)
    response_dict = response.json()
...
def test_put_developer_setting_invalid(client, api_prefix):
...
def test_put_viewer_setting_invalid(client, api_prefix):
...
def test_put_viewer_setting_invalid_key(client, api_prefix):
    viewer_settings_dict = {
        "key": "invalid_key",
        "value": "Testy Mc Test Face"
        }
