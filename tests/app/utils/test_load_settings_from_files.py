...
# Utility to load test JSON data from disk
def load_test_json(filename: str) -> dict:
    path = Path("tests/app/utils/data") / filename
...
def temp_settings_files(tmp_path, monkeypatch):
    ...
    # Patch the module paths to use the temp files
...
def test_load_merged_settings(temp_settings_files):
    ...
    # Your test assertions depend on the test JSON contents
