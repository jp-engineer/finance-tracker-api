...
if MODE == "e2e_testing":
    DB_PATH = os.path.join("tests", "data", "e2e", "db", DB_FILENAME)
    SEED_DIR = os.path.join("tests","data", "e2e", "db", "seed")
    ...
    if DB_SEED_FILE is None:
        ...
        if seed_file_str is None:
            logger.error(f"Invalid DB_SEED_FILE: {DB_SEED_FILE}. Exiting.")
            sys.exit(1)
        DB_SEED_FILE = seed_file_str

    if SETTINGS_FILE is None:
        SETTINGS_FILE = os.path.join("tests", "data", "e2e", "user", "test_user_settings.yml")
